...
class RoomControllerTest {
    ...
    void testGetThematicRooms_Success() {
        ...
        thematicRoom1.setName("Thematic Room 1");
        thematicRoom1.setType(Room.RoomType.THEMATIC);

        when(roomService.getThematicRooms()).thenReturn(Arrays.asList(thematicRoom1));
    ...
    @Test
    void testMarkInvitationsSeen_SetSeenAtTimestamp() {
        testRoomMember.setSeenAt(null);
        when(userRepository.findByUsername("testuser")).thenReturn(Optional.of(testUser));
        when(roomMemberRepository.findByUserIdAndSeenAtIsNull(1L))
                .thenReturn(Arrays.asList(testRoomMember));
        ...
        verify(roomMemberRepository, times(1)).save(argThat(member ->
            member.getSeenAt() != null && member.getSeenAt() instanceof Instant
        ));
    }
    ...
    @Test
    void testCreatePrivateRoom_ReturnsCorrectStatus() {
        CreatePrivateRoomRequestDTO request = new CreatePrivateRoomRequestDTO("Room", Arrays.asList());

        when(roomService.createPrivateRoom("Room", "testuser", Arrays.asList()))
                .thenReturn(testRoom);
